...
save_path = os.getcwd() + "/AB/Moon_CNOT_simulations/figures/"
...
ax[0, 1].legend()
fig.tight_layout()
plt.savefig(save_path + "kappa_a_study.png")
...
for i, sub in enumerate(["X", "Z"]):
    for j, sur in enumerate(["C", "T"]):
        ...
        ax[i, j].set_xscale("log")
        ax[i, j].set_yscale("log")
        ax[i, j].set_xlabel(r"$\kappa_\phi/\kappa_2$")
        ax[i, j].set_ylabel(f"$\\varepsilon_{sub}^{sur}$")
        ax[i, j].grid()
fig.tight_layout()
ax[0, 1].legend()
plt.savefig(save_path + "kappa_phi_study.png")

# Kerr study
kappa_a = 10 * kHz
kappa_phi = 10 * kHz
kerr = kappa_2 * jnp.geomspace(1e-3, 1e-1, 11) * 5
...
for i, sub in enumerate(["X", "Z"]):
    for j, sur in enumerate(["C", "T"]):
        ...
        ax[i, j].set_xscale("log")
        ax[i, j].set_yscale("log")
        ax[i, j].set_xlabel(r"$K_4/\kappa_2$")
        ax[i, j].set_ylabel(f"$\\varepsilon_{sub}^{sur}$")
        ax[i, j].grid()
fig.tight_layout()
ax[0, 1].legend()
plt.savefig(save_path + "kerr_study.png")
